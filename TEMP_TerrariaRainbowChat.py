...
def modifyGradientColor(activeGradientColor, targetGradientColor):
	...
	try:
		newValue = Color(input("Input a new colour: "))
	except:
...
def main():
	...
	if (updateCheck.status_code != 200):
		print("There was an error getting to the update server. Check your internet connection.")
		...
		if(version.parse(versionNumber) > version.parse(__version__)):
			...
			if(input("New version found! Do you wish to update? (YES/no): ").lower() == "yes"):
